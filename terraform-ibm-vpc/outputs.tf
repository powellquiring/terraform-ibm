output "psac" {
  value = var.private_subnets_address_count
}

output "vpc_id" {
  description = "The ID of the VPC"
  value       = element(concat(ibm_is_vpc.this.*.id, [""]), 0)
}

output "default_security_group" {
  description = "The ID of the security group created by default on VPC creation"
  value       = element(concat(ibm_is_vpc.this.*.default_security_group, [""]), 0)
}
...
output "private_subnets" {
  ...
  value       = flatten([ibm_is_subnet.private.*.id, ibm_is_subnet.private_address_count.*.id])
}

output "private_ipv4_cidr_blocks" {
  description = "List of cidr blocks associated with private subnets"
  value       = flatten([ibm_is_subnet.private.*.ipv4_cidr_block, ibm_is_subnet.private_address_count.*.ipv4_cidr_block])
}
